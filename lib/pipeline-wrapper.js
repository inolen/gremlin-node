...
PipelineWrapper.prototype.count = function (callback) {
  this.pipeline.count(function (err, res) {
    ...
    callback(null, parseInt(res.longValue, 10));
  });
};
...
PipelineWrapper.prototype.next = function (callback) {
  ...
  this.pipeline.next(function (err, res) {
    ...
    if (res) {
      if (gremlin.isType(res, 'com.tinkerpop.blueprints.Vertex')) {
        res = new gremlin.VertexWrapper(gremlin, res);
      } else if (gremlin.isType(res, 'com.tinkerpop.blueprints.Edge')) {
        res = new gremlin.EdgeWrapper(gremlin, res);
      }
    }
...
PipelineWrapper.prototype.toArray = function (callback) {
  ...
  this.pipeline.toList(function (err, list) {
    ...
    for (var i = 0, l = list.sizeSync(); i < l; i++) {
      ...
      if (gremlin.isType(it, 'com.tinkerpop.blueprints.Vertex')) {
        it = new gremlin.VertexWrapper(gremlin, it);
      } else if (gremlin.isType(it, 'com.tinkerpop.blueprints.Edge')) {
        it = new gremlin.EdgeWrapper(gremlin, it);
      } else {
