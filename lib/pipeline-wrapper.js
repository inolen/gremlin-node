...
PipelineWrapper.prototype.count = function () {
  var argPair = this.gremlin.extractArguments(Array.prototype.slice.call(arguments));
  var self = this;
  return Q.npost(this.pipeline, 'count', argPair.args)
    .then(function(res) { return new Q(self._jsify(res)); })
    .nodeify(argPair.callback);
};

// PipelineWrapper.prototype.count = pipePromiseWrap('count');
...
PipelineWrapper.prototype.next = function (callback) {
  dlog('PipelineWrapper.next.start');
  var self = this;

  this.pipeline.next(function (err, res) {
    dlog('PipelineWrapper.next.done');
    if (err) return callback(err);
    callback(null, self._jsify(res));
  });
};

PipelineWrapper.prototype.nextP = function () {
  return Q.nbind(this.next, this)();
};
...
PipelineWrapper.prototype.toArrayP = function () {
  return Q.nbind(this.toArray, this)();
};
...
PipelineWrapper.prototype.toJSONP = function () {
  return Q.nbind(this.toJSON, this)();
};
