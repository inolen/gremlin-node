...
Gremlin.prototype.wrap = function (val) {
  if (val.longValue) {
    ...
    try {
      ...
      // entrySet iterators are unavailable in Java 1.6 (?)
      var keys = val.keySetSync().toArraySync();
      var vals = val.valuesSync().toArraySync();
      while (keys.length > 0) {
        map[keys.shift()] = this.wrap(vals.shift());
      }
    }
    return map;
  }
